package com.albionhelper.helper.tierList;
...
public class TierListService {
    ...
    public TierListService(TierListRepository tierListRepository, TierListCommentRepository tierListCommentRepository) {
    ...
    public String register(TierListDTO dto) {
        ...
        try{
            tierListRepository.save(entity);
            return "저장이 완료되었습니다.";
        }catch (Exception e){
    ...
    public Page<TierListDTO> getTierList(String category, String sortBy, int page, int size) {
        ...
        if (sortBy.equals("tierListComments")) {
            ...
            Page<TierList> entityList = tierListRepository.findAllByCategoryOrderByCommentCountDesc(category, pageable);
            return entityList.map(TierList::toDTO);
        }else{
            Sort sort = Sort.by(sortBy).descending();
            Pageable pageable = PageRequest.of(page, size, sort);
            Page<TierList> entityList = tierListRepository.findAllByCategory(category, pageable);
            return entityList.map(TierList::toDTO);
        }
    }

    public TierListDTO getTierListById(Long id) {
        Optional<TierList> optional = tierListRepository.findById(id);
        return optional.get().toDTO();
    }

    public String registerComment(TierListCommentDTO dto) {
        TierList tierList = tierListRepository.findById(dto.getTierListId()).get();
    ...
    public String modifyUp(Long id) {
        Optional<TierList> byId = tierListRepository.findById(id);
        if(byId.isPresent()){
