...
public class BattleBoardController {
    ...
    @Autowired
    BattleBoardService battleBoardService;
    ...
    public String getBattles(Model model,
        ...
        battle = battleBoardService.getPlayers(battle);

        // 특정 전투의 이벤트(킬 로그)들을 전부 종합해서 가져오는 메서드.
        List<Event> eventList = battleBoardService.getEventList(id, server, battle.getTotalKills());


        // 모스트 킬
        Player p = battleBoardService.getMostKillingPlayer(battle);

        // 플레이어 리스트
        Map<String, EventPlayer> playerList = battleBoardService.getPlayerList(eventList, battle);

        // 길드별, 연합별 평균 ip구하기.
        battle = battleBoardService.getAverageIp(battle, playerList);

        // [모스트 데미지, 모스트 힐, 빅도네이션]
        EventPlayer[] players = battleBoardService.getMostDpsAndHeal(playerList);

        model.addAttribute("playerList", battleBoardService.sortAndConvertMapToList(playerList));
