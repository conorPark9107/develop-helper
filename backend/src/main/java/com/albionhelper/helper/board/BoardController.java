...
//    }
    ...
    public ResponseEntity<Void> register(@RequestBody BoardRequestDTO dto) throws UnknownHostException {
        log.info("register board => {}", dto);
        boardService.registerBoard(dto);
    ...
    public ResponseEntity<Map<String, Object>> registerComment(@RequestBody CommentRequestDTO dto) throws UnknownHostException {

        log.info("CommentRequestDTO : {}", dto.toString());

        boardService.registerComment(dto);

        // 클라이언트에 JSON으로 전달
        Map<String, Object> response = new HashMap<>();
        response.put("board_id", dto.getBoard_id());
