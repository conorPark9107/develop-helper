...
public class BattleBoardControllerV2 {

    private final Logger log = LoggerFactory.getLogger(this.getClass());

    @Autowired
    private final BattleBoardServiceV2 battleBoardServiceV2;

    public BattleBoardControllerV2(BattleBoardServiceV2 battleBoardServiceV2) {
        this.battleBoardServiceV2 = battleBoardServiceV2;
    }
    ...
    public String showPage(Model model
        ...
        model.addAttribute("count", battleBoardServiceV2.getCount(url));
        List<Battle> list = battleBoardServiceV2.getBattleList(url, offset, limit, inputValue);
    ...
    public List<BattleCountLogDTO> getCount(@RequestParam(name = "server", defaultValue = "") String server){
        return battleBoardServiceV2.getCount(server);
    }
    ...
    public List<Battle> refreshList(
        ...
        return battleBoardServiceV2.getBattleListForRefresh(url, inputValue, recentId);
    }
    ...
    public List<Battle> showPageForAjax(
        ...
        if(!guildName.isEmpty()){
            battleBoardServiceV2.addGuildCount(inputValue, url, guildName);
        }
        return battleBoardServiceV2.getBattleList(url, offset, limit, inputValue);
    }
    ...
    public String getBattles(Model model,
        ...
        Battle battle = battleBoardServiceV2.getBattleListById(id, server);

        // 플레이어 인원을 길드와 연합에 채워넣는 메서드.
        battle = battleBoardServiceV2.getPlayers(battle);

        // 특정 전투의 이벤트(킬 로그)들을 전부 종합해서 가져오는 메서드.
        List<Event> eventList = battleBoardServiceV2.getEventList(id, server, battle.getTotalKills());

        // 모스트 킬.
        Player p = battleBoardServiceV2.getMostKillingPlayer(battle);

        // 플레이어 리스트.
        Map<String, EventPlayer> playerList = battleBoardServiceV2.getPlayerList(eventList, battle);

        // 길드별, 연합별 평균 ip구하기.
        battle = battleBoardServiceV2.getAverageIp(battle, playerList);

        // [모스트 데미지, 모스트 힐, 빅도네이션]
        EventPlayer[] players = battleBoardServiceV2.getMostDpsAndHeal(playerList);

        model.addAttribute("playerList", battleBoardServiceV2.sortAndConvertMapToList(playerList));
...
//    }
    ...
    public List<GuildDTO> getGuildId(@RequestParam(name = "id", defaultValue = "") String id
        ...
        return battleBoardServiceV2.getGuildId(id, server);
    }
